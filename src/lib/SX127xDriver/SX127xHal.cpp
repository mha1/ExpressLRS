...
uint8_t ICACHE_RAM_ATTR SX127xHal::readRegisterBits(uint8_t reg, uint8_t mask, Radio_Number_t radioNumber)
{
...
uint8_t ICACHE_RAM_ATTR SX127xHal::readRegister(uint8_t reg, Radio_Number_t radioNumber)
{
    uint8_t data;
...
void ICACHE_RAM_ATTR SX127xHal::readRegister(uint8_t reg, uint8_t *data, uint8_t numBytes, Radio_Number_t radioNumber)
{
...
void ICACHE_RAM_ATTR SX127xHal::writeRegisterBits(uint8_t reg, uint8_t value, uint8_t mask, Radio_Number_t radioNumber)
{
    if (radioNumber & Radio_1)
    {
        uint8_t currentValue = readRegister(reg, Radio_1);
        uint8_t newValue = (currentValue & ~mask) | (value & mask);
        writeRegister(reg, newValue, Radio_1);
    }

    if (GPIO_PIN_NSS_2 != UNDEF_PIN && radioNumber & Radio_2)
    {
        uint8_t currentValue = readRegister(reg, Radio_2);
        uint8_t newValue = (currentValue & ~mask) | (value & mask);
        writeRegister(reg, newValue, Radio_2);
    }
}

void ICACHE_RAM_ATTR SX127xHal::writeRegister(uint8_t reg, uint8_t data, Radio_Number_t radioNumber)
{
    writeRegister(reg, &data, 1, radioNumber);
}

void ICACHE_RAM_ATTR SX127xHal::writeRegister(uint8_t reg, uint8_t *data, uint8_t numBytes, Radio_Number_t radioNumber)
{
    WORD_ALIGNED_ATTR uint8_t buf[WORD_PADDED(numBytes + 1)];
    buf[0] = reg | SPI_WRITE;

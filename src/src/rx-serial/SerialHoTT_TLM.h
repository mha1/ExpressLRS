...
typedef struct
{
    ...
    uint8_t packetId;       //  1 0x8D HOTT_General_AIR_PACKET_ID
    uint8_t warnBeep;       //  2 warn beep (0 = no beep, 0x00..0x1A warn beeps)
    uint8_t packetIdText;   //  3 0xD0 Sensor ID text mode
    uint8_t inverse1;       //  4 Alarm_inverse_1
    uint8_t inverse2;       //  5 Alarm_inverse_2
    uint8_t voltageCell1;   //  6 124 = 2,48V (voltage*50)
    uint8_t voltageCell2;   //  7 132 = 2,64V
    uint8_t voltageCell3;   //  8
    uint8_t voltageCell4;   //  9
    uint8_t voltageCell5;   // 10
    uint8_t voltageCell6;   // 11
    uint16_t battery1;      // 12 51 = 5,1V Batt1 voltage
    uint16_t battery2;      // 14 51 = 5,1V Batt2 voltage
    uint8_t temperature1;   // 16 46 = 26C, 0 = -20C (temp+20)
    uint8_t temperature2;   // 17 45 = 25C, 0 = -20C (temp+20)
    uint8_t fuelScale;      // 18 0..100 (scales to display 0/25/50/75/100 increments)
    uint16_t fuel;          // 19 65535 = full
    uint16_t rpm1;          // 21 (rpm/10)
    uint16_t altitude;      // 23 500 = 0m (altitude+500)
    uint16_t mPerSec;       // 25 1 = 0,01m/s 30000 = 0m/s (climb rate/100 + 30000)
    uint8_t mPer3sec;       // 27 120 = 0m per 3 sek
    uint16_t current;       // 28 1 = 0.1A (current*10)
    uint16_t inputVoltage;  // 30 166 = 16,6V (voltage*10)
    uint16_t capacity;      // 32 1 = 10mAh (capacity in mAh/10)
    uint16_t speed;         // 34 1 = 2km/h (speed/2)
    uint8_t lowCellVoltage; // 36 124 = 2,48V (voltage*50)
    uint8_t cellNumberLcv;  // 37
    uint16_t rpm2;          // 38 (rpm/10)
    uint8_t generalError;   // 40 general error number (voice error = 12)
    uint8_t pressure;       // 41 pressure 20 = 2bar (pressure*10)
...
{
    uint8_t startByte;        //  0 0x7C
    uint8_t packetId;         //  1 0x8C HOTT_General_AIR_PACKET_ID
    uint8_t warnBeep;         //  2 warn beep (0 = no beep, 0x00..0x1A warn beeps)
    uint8_t packetIdText;     //  3 0xC0 Sensor ID text mode
    uint8_t inverse1;         //  4 TODO: more info
    uint8_t inverse2;         //  5 TODO: more info
    uint16_t inputVoltage;    //  6 Input voltage
    uint16_t inputVoltageMin; //  8 Input min. voltage
    uint16_t capacity;        // 10 battery capacity in 10mAh steps
    uint8_t escTemp;          // 12 ESC temperature
    uint8_t escMaxTemp;       // 13 ESC max. temperature
    uint16_t current;         // 14 Current in 0.1 steps
    uint16_t currentMax;      // 16 Current max. in 0.1 steps
    uint16_t rpm;             // 18 RPM in 10U/min steps
    uint16_t rpmMax;          // 20 RPM max
    uint8_t motorTemp;        // 22 motor temp
    uint8_t motortempMax;     // 23 motor temp max
    uint16_t speed;           // 24 Speed
    uint16_t speedMax;        // 26 Speed max
    uint8_t pwm;              // 28 PWM in %
    uint8_t throttle;         // 29 throttle in %
    uint8_t becVoltage;       // 30 BEC voltage
    uint8_t becVoltageMin;    // 31 BEC voltage min
    uint16_t becCurrent;      // 32 BEC current
    uint8_t becTemp;          // 34 BEC temperature
    uint8_t capacitorTemp;    // 35 Capacitor temperature
    uint8_t motorTiming;      // 36 motor timing
    uint8_t auxTemp;          // 37 advanced timing/aux temp
    uint16_t thrust;          // 38 thrust/force
    uint8_t pumpTemp;         // 40 pump temperature
    uint8_t turbineNumber;    // 41 engine number (200 = #1, 201 = #2)
